	...
	{	
		...
		if(ans==1)
		{
			...
			while(k<100)	
			{
				...
				cout << "\nMulticast Message Sent from <Machine (" << myProcID << ")>" << endl;
				cout << "\n---------------------------------------------" << endl;
				cout << "\tVector Clock: (";
				for(int i = 0; i < (total_p - 1); i++)
				{
...
int main(int argc, char *argv[])
{
	...
	while(cnt < total_p)							// Waiting till all the processes are connected
	{
		continue;
	}
...
//--------------------Connections Completed ---------------------------------------
	...
	for(int i=0; i < (total_p-1); i++)						// Sorting the processes according to their ID values
	{
	...
	for(int i = 0; i < total_p; i++)						// Printing the Process IDs and respective Port Numbers
	{
	...
	for(int i=0; i < total_p; i++)							// Spawning Multicast Receive Threads
	{
		if(i == (myProcID - 1))
		{
			continue;
